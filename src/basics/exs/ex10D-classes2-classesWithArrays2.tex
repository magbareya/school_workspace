...
% %=============================================================
% \section{مثال توضيحي -- حجوزات رحلة طائرة}
% %=============================================================
...
% %--- سؤال 4 ---
% %=============================================================
% \item \textbf{محطة أرصاد جوية}

% اكتب فئة \textenglish{WeatherStation} تمثل محطة رصد مناخي.
% تتلقى العملية البناءة اسم المحطة (\textenglish{string}) وعدد أيام الشهر (\textenglish{int})،
% وتُنشئ مصفوفة من نوع \textenglish{double} بنفس الحجم وتُهيئ جميع قيمها بـ\textenglish{-999}
% للدلالة على أن درجة حرارة ذلك اليوم لم تُسجَّل بعد.
% على الفئة أن تدعم العمليات التالية:

% \begin{itemize}
%   \item \textenglish{RecordTemp(int dayIndex, double temp) : void} --
%         تُسجّل درجة الحرارة \textenglish{temp} لليوم ذي الرقم \textenglish{dayIndex}.
%         لا تأثير إذا كان الرقم خارج النطاق.

%   \item \textenglish{MaxTemp() : double} --
%         تُعيد أعلى درجة حرارة مسجّلة في الشهر،
%         أو \textenglish{-999} إذا لم يُسجَّل أي يوم بعد.

%   \item \textenglish{AverageTemp() : double} --
%         تُعيد معدل درجات الحرارة للأيام المسجّلة فقط،
%         أو \textenglish{-999} إذا لم يُسجَّل أي يوم بعد.

%   \item \textenglish{DaysAbove(double threshold) : int} --
%         تتلقى حداً معيناً وتُعيد عدد الأيام المسجّلة التي تجاوزت درجة حرارتها هذا الحد.
% \end{itemize}

% اكتب في \textenglish{Main} كوداً ينشئ محطة باسم \textenglish{"Haifa"} لشهر مدته 5 أيام،
% يُسجّل الدرجات: 28، 31، 25، 35، 29،
% ثم يطبع أعلى درجة والمعدل وعدد الأيام فوق 30 درجة.
...
%     private const double NOT_RECORDED = -999;

%     public WeatherStation(string stationName, int daysInMonth)
%     {
%         this.stationName = stationName;
%         this.temps       = new double[daysInMonth];
%         for (int i = 0; i < temps.Length; i++)
%             temps[i] = NOT_RECORDED;
%     }

%     public void RecordTemp(int dayIndex, double temp)
%     {
%         if (dayIndex < 0 || dayIndex >= temps.Length)
%             return;
%         temps[dayIndex] = temp;
%     }
...
%     {
%         double max = NOT_RECORDED;
%         for (int i = 0; i < temps.Length; i++)
%             if (temps[i] != NOT_RECORDED && temps[i] > max)
%                 max = temps[i];
%         return max;
%     }
...
%         int    count = 0;
%         for (int i = 0; i < temps.Length; i++)
%             if (temps[i] != NOT_RECORDED)
%             {
%                 sum += temps[i];
%                 count++;
%             }
%         if (count == 0) return NOT_RECORDED;
%         return sum / count;
%     }

%     public int DaysAbove(double threshold)
%     {
%         int count = 0;
%         for (int i = 0; i < temps.Length; i++)
%             if (temps[i] != NOT_RECORDED && temps[i] > threshold)
%                 count++;
%         return count;
%     }
% }
...
% // In Main
% WeatherStation ws = new WeatherStation("Haifa", 5);
% ws.RecordTemp(0, 28);
% ws.RecordTemp(1, 31);
% ws.RecordTemp(2, 25);
% ws.RecordTemp(3, 35);
% ws.RecordTemp(4, 29);
% Console.WriteLine("Max: "        + ws.MaxTemp());        // 35
% Console.WriteLine("Average: "    + ws.AverageTemp());    // 29.6
% Console.WriteLine("Days > 30: "  + ws.DaysAbove(30));    // 2
% \end{minted}
% }
% \end{english}
% \end{boxSolution}
